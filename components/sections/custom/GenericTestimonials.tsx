...
export function GenericTestimonials({ items, onChange }: { items: Item[]; onChange: (items: Item[]) => void }) {
  ...
  return (
    <div className="space-y-3">
      <button type="button" onClick={add} className="rounded-lg border border-admin-border px-3 py-2 text-sm">Add Testimonial</button>
      {items.map((item, index) => (
        ...
        >
          <div className="grid gap-3 md:grid-cols-2">
            {["name", "role", "company", "rating", "quote"].map((key) => (
              <label key={key} className="space-y-1 text-sm md:col-span-2">
                <span className="font-medium text-admin-text">{key}</span>
                <textarea value={String(item[key] || "")} onChange={(e) => update(index, { [key]: e.target.value })} rows={key === "quote" ? 4 : 2} className="w-full rounded-xl border border-admin-border bg-admin-input px-3 py-2 text-admin-text" />
              </label>
            ))}
            <label className="space-y-1 text-sm md:col-span-2">
              <span className="font-medium text-admin-text">Avatar</span>
              <div className="flex items-center gap-3">
                {item.avatar ? <img src={item.avatar} alt={String(item.name || "")} className="h-16 w-16 rounded-full object-cover" /> : null}
                <input type="file" accept="image/*" onChange={async (e) => {
                  const file = e.target.files?.[0];
                  if (!file) return;
                  const url = await uploadImage(file);
                  update(index, { avatar: url });
                  e.currentTarget.value = "";
                }} className="w-full rounded-xl border border-admin-border bg-admin-input px-3 py-2 text-admin-text" />
                {item.avatar ? <button type="button" onClick={() => update(index, { avatar: "" })} className="rounded-lg border border-admin-border px-3 py-2 text-sm">Remove</button> : null}
              </div>
            </label>
