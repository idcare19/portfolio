...
export function GlobalPopupAnnouncement() {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-[rgba(15,23,42,0.18)] p-4">
      <div
        className={`relative w-full max-w-lg overflow-hidden rounded-3xl border bg-gradient-to-b p-0 shadow-[0_24px_60px_rgba(15,23,42,0.12)] ${styleMap[config.style] || styleMap.info}`}
      >
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {config.buttonText && config.buttonLink ? (
              <a
                ...
                className="rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white shadow-[0_10px_20px_rgba(37,99,235,0.2)] transition-transform hover:-translate-y-0.5"
              >
                {config.buttonText}
              </a>
            ) : null}
