  ...
  async function pull(config: GitHubConfigInput) {
    ...
    try {
      ...
      if (!res.ok || !payload?.ok) {
        throw new Error(payload?.error || payload?.message || "Content load failed");
      }

      return { ok: true as const, data: payload?.data as SiteData };
    } catch (error) {
      const message = error instanceof Error ? error.message : "Content load failed";
  ...
  async function push(config: GitHubConfigInput, data: SiteData, commitMessage: string) {
    ...
    try {
      const res = await fetch("/api/admin/content", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "push", data, commitMessage }),
      });
      const payload = await res.json();

      if (!res.ok || !payload?.ok) {
        throw new Error(payload?.error || payload?.message || "Content save failed");
      }
