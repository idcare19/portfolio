...
const DEFAULT_VERCEL_DEPLOY_HOOK_URL = process.env.VERCEL_DEPLOY_HOOK_URL;
...
async function githubRequestWithResponse(url: string, config?: Partial<GitHubProjectConfig>, init?: RequestInit) {
  assertEnv(config);
  const res = await fetch(url, {
    ...init,
    headers: githubHeaders(config, init?.headers),
  ...
  if (!res.ok) {
    ...
    throw new Error(`GitHub API error (${res.status}): ${details || "No response body"}`);
  }
...
export async function updateGitHubJsonFile({
  ...
  let current = await readGitHubJsonFile(contentPath, projectConfig);
  let lastError: Error | null = null;

  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      const { json } = await githubRequestWithResponse(githubUrl(contentPath, projectConfig), projectConfig, {
        method: "PUT",
        body: JSON.stringify({
          message,
          content,
      ...
      return {
        ...(json as { content: { sha: string; path?: string }; commit: { sha: string } }),
      ...
      lastError = error instanceof Error ? error : new Error("GitHub update failed");
      const lower = lastError.message.toLowerCase();
      const isShaConflict = lower.includes("(409)") || lower.includes("sha") || lower.includes("does not match");

      if (!isShaConflict || attempt === 1) {
        throw lastError;
      }

      current = await readGitHubJsonFile(contentPath, projectConfig);
    }
  }
...
export async function triggerVercelDeployHook() {
  if (!DEFAULT_VERCEL_DEPLOY_HOOK_URL) {
    return {
      ok: false as const,
      skipped: true as const,
      reason: "VERCEL_DEPLOY_HOOK_URL is not configured",
    };
  }

  const res = await fetch(DEFAULT_VERCEL_DEPLOY_HOOK_URL, {
    method: "POST",
    cache: "no-store",
  });
  const text = await res.text();
  ...
  if (!res.ok) {
    throw new Error(`Vercel deploy hook failed (${res.status}): ${text || "No response body"}`);
  }
